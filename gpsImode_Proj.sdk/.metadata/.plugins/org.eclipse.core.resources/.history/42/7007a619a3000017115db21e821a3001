...
void TDC_config(void)
{
	TDC_RegConfig(TDC_REG_0, 0xAB);
	TDC_RegConfig(TDC_REG_1, 0x5050500);
	TDC_RegConfig(TDC_REG_2, 0x50001);
	TDC_RegConfig(TDC_REG_3, 0x8000000);
	TDC_RegConfig(TDC_REG_4, MTIMER_START);
	TDC_RegConfig(TDC_REG_5, ALU_MASTER_RST | STOP_DIS_START | START_OFF_1);
	TDC_RegConfig(TDC_REG_6, 0x0100000);
	TDC_RegConfig(TDC_REG_7, HS_DIV | REF_CLK_DIV | RES_ADJ | NEG_PHASE | MTIMER);
	TDC_RegConfig(TDC_REG_11, 0x7FF0000);
	TDC_RegConfig(TDC_REG_12, 0x2000000);
	TDC_RegConfig(TDC_REG_14, 0x0000000);
	TDC_RegConfig(TDC_REG_4, MTIMER_START | MASTER_RST);
	return;
...
#else
unsigned int chandata[FIFO_SIZE][4]={{0,0,0,0}};
void TDC_TimeDataAllocate_test(void){
    int i;
    int status = 0;
    xil_printf("Tstart status :%d\r\n", MYIMODE_mReadReg(TDC_BASEADDR, 8));
        ...
        for(i = 0; i < FIFO_SIZE; i++){
            while(1){
                ...
                usleep(1);
                 ...
                 chandata[i][0] = MYIMODE_mReadReg(TDC_BASEADDR, 36);
                 chandata[i][1] = MYIMODE_mReadReg(TDC_BASEADDR, 40);
                 chandata[i][2] = MYIMODE_mReadReg(TDC_BASEADDR, 8);//tstart_counter
                 chandata[i][3] = MYIMODE_mReadReg(TDC_BASEADDR, 20);//ef1
                 while(1){
                 	status = MYIMODE_mReadReg(TDC_BASEADDR, 0);//check stopDis, if it became 1, then the data can be read.
                     if(status == 0){
                         break;
        	...
        	for(i = 0; i<FIFO_SIZE; i+= 20){
        		xil_printf("ch1:%d\tch2:%d\ttri:%d\tef1:%d\r\n",
        				chandata[i][0], chandata[i][1], chandata[i][2], chandata[i][3]);
        	}
