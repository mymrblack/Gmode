...
void FIFO_getData(Lidar_Data *data, int fifoNum){
    if(fifoNum == 1){
        ...
        #else
        data->delay1 = FIFO_mReadReg(FIFO_BASEADDR, FIFO1_DELAY1_DATA_REG);
        data->delay2 = FIFO_mReadReg(FIFO_BASEADDR, FIFO1_DELAY2_DATA_REG);
        data->delay3 = FIFO_mReadReg(FIFO_BASEADDR, FIFO1_DELAY3_DATA_REG);
        data->delay4 = FIFO_mReadReg(FIFO_BASEADDR, FIFO1_DELAY4_DATA_REG);
        data->delay4 = FIFO_mReadReg(FIFO_BASEADDR, FIFO1_DELAY4_DATA_REG);
        data->motor = FIFO_mReadReg(FIFO_BASEADDR, FIFO1_MOTOR_DATA_REG);
        #endif
        ...
        #else
        data->delay1 = FIFO_mReadReg(FIFO_BASEADDR, FIFO2_DELAY1_DATA_REG);
        data->delay2 = FIFO_mReadReg(FIFO_BASEADDR, FIFO2_DELAY2_DATA_REG);
        data->delay3 = FIFO_mReadReg(FIFO_BASEADDR, FIFO2_DELAY3_DATA_REG);
        data->delay4 = FIFO_mReadReg(FIFO_BASEADDR, FIFO2_DELAY4_DATA_REG);
        data->motor = FIFO_mReadReg(FIFO_BASEADDR, FIFO2_MOTOR_DATA_REG);
        #endif
...
void FIFO_printData(Lidar_Data *data, int range, int step){
    ...
    #else
    for(i = 0; i < range; i+= step){
        xil_printf("%d\tgps1:%d\tgps2:%d\tch1:%d\tch2:%d\tdelay1:%d\tdelay2:%d\tdelay3:%d\tdelay4:%d\tmotor:%d\t\r\n",
        		(&data[i])->triTimes, (&data[i])->gps1, (&data[i])->gps2,
