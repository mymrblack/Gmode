...
void motor_run_config(Motor_Data *motor){
    int move_time_p = 0;
    int move_time_n = 0;
    int dir_time_p = 0;
    int dir_time_n = 0;
...
void motor_run_en(void){
    lidar_mWriteReg(MOTOR_BASEADDR, MOTOR_CTRL_REG, 1 << RUN_EN_BIT);
}

void motor_run_disable(void){
    lidar_mWriteReg(MOTOR_BASEADDR, MOTOR_CTRL_REG, 0 << RUN_EN_BIT);
}
